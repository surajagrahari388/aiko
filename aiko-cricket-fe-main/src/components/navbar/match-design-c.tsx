...
/**
 * Design C — Bottom score bar (exact copy of Design A layout, positioned at bottom).
 */
export default function MatchDesignC({
  teamAName,
  teamAShortName,
  teamALogo,
  teamAScore,
  ...
  const teamAParsed =
    teamAScore && teamAScore.trim() ? parseScore(teamAScore) : null;
  const teamBParsed =
    teamBScore && teamBScore.trim() ? parseScore(teamBScore) : null;
  ...
  return (
    <div className="w-full bg-background border-t border-border">
      <div className="container mx-auto px-3 sm:px-6 lg:px-6 py-3 sm:py-3.5 flex flex-col gap-3 sm:gap-3.5">
        {/* Main section */}
        <div>
          <div className={`flex ${hasScores ? 'items-start' : 'items-center'} justify-between gap-1.5 sm:gap-2`}>
          {/* Team A */}
          <div className={`flex ${hasScores ? 'items-start' : 'items-center'} gap-1.5 sm:gap-2 flex-1 min-w-0`}>
            ...
            <div className="flex flex-col min-w-0">
              ...
              {teamAParsed ? (
                <span className="text-lg sm:text-2xl md:text-3xl font-bold text-foreground leading-tight whitespace-nowrap">
                  {teamAParsed.score}
                </span>
              ) : isLive ? (
          ...
          {/* Center: ball event or time/date */}
          <div className="shrink flex flex-col items-center px-1 sm:px-4 max-w-[35%] sm:max-w-none pt-0.5">
            {isLive && lastBallLabel ? (
              ...
              >
                <Badge
                  className={`relative ${lastBallBadgeColor} text-white text-xs sm:text-xl font-bold px-3 sm:px-6 py-1.5 sm:py-2.5 border-0 rounded-full uppercase tracking-wider shadow-lg whitespace-nowrap`}
                >
                  {lastBallType === "wicket"
                    ? "OUT!"
                    : lastBallType === "six"
                      ? "SIX!"
                      : lastBallType === "wide+six"
                        ? "WIDE SIX!"
                        : lastBallType === "noball+six"
                          ? "NB SIX!"
                          : lastBallType === "four"
                            ? "FOUR!"
                            : lastBallType === "wide+four"
                              ? "WIDE FOUR!"
                              : lastBallType === "noball+four"
                                ? "NB FOUR!"
                                : lastBallType === "wide"
                                  ? "WIDE"
                                  : lastBallType === "noball"
              ...
              <div className="flex flex-col items-center gap-0.5 text-xs sm:text-sm text-muted-foreground">
                ...
                {dateStr && (
                  <span className="font-medium text-[10px] sm:text-xs">{dateStr}</span>
                )}
              </div>
            )}
          </div>
